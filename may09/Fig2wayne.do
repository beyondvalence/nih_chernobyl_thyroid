...
gen ctpoint=.
replace ctpoint=1 if age_exp10==6|age_exp10==15|age_exp10==72 /*72 for now*/

gen exct3rad=   ((agecat1b  *agecat1 + agecat2b  *agecat2 + agecat3b  *agecat3 ) * ctpoint) + 0
gen exct3hirad= ((agecat1hib*agecat1 + agecat2hib*agecat2 + agecat3hib*agecat3 ) * ctpoint) + 0
gen exct3lorad= ((agecat1lob*agecat1 + agecat2lob*agecat2 + agecat3lob*agecat3 ) * ctpoint) + 0 /// exp and + 1 
...
twoway	(rcap exct3hirad exct3lorad age_exp) ///
		...
		(line linrad  age_exp, ///
		 ...
		 lw(medthick )), ///
			...
			yti("{bf}Odds ratio*", size(4.7) )  yla(,ang(0)) ylab(0 (2) 16) ///
...
scalar agecat1_ben_b=1.28
scalar agecat1_mal_b=2.19722 /// 6.27

scalar agecat2_ben_b=1.08
scalar agecat2_mal_b=2.19722 /// 4.91 

scalar agecat3_ben_b=1.07
scalar agecat3_mal_b=2.19722 /// 1.57

gen ctpoint=.
replace ctpoint=1 if age_exp10==6|age_exp10==15|age_exp10==35

gen exct3_ben_rad= exp((agecat1_ben_b*agecat1 + agecat2_ben_b*agecat2 + agecat3_ben_b*agecat3 ) * ctpoint) + 1
gen exct3_mal_rad= exp((agecat1_mal_b*agecat1 + agecat2_mal_b*agecat2 + agecat3_mal_b*agecat3 ) * ctpoint) + 1

!Modelled effect modification

scalar dgy_ben_b= 0.83/// 0.4859
scalar dgy_ben_leb=-0.19
scalar ex5_ben_b=-0.2162
gen lerad_ben= dgy_ben_b*dose1*exp(ex5_ben_b*(age_exp-5)+dgy_ben_leb*dose1)+1

scalar dgy_mal_b=0.79
scalar ex5_mal_b=-0.2908
gen linrad_mal= dgy_mal_b*dose1*exp(ex5_mal_b*(age_exp-5))+1

twoway	(scatter exct3_ben_rad age_exp, mc(gray)) ///
		(line linrad_ben  age_exp, ///
		 lpattern( solid ) ///
		 lcol(gray*0.75 ) ///
		 lw(medthick )) ///
		(scatter exct3_mal_rad age_exp, mc(black)) ///
...
scalar agecat1_lar_b=2.19722 /// 3.85

scalar agecat2_sma_b=1.43
scalar agecat2_lar_b=2.19722 /// 2.25
...
gen exct3_sma_rad= exp((agecat1_sma_b*agecat1 + agecat2_sma_b*agecat2 + agecat3_sma_b*agecat3 ) * ctpoint) + 1
gen exct3_lar_rad= exp((agecat1_lar_b*agecat1 + agecat2_lar_b*agecat2 + agecat3_lar_b*agecat3 ) * ctpoint) + 1

!Modelled effect modification

scalar dgy_sma_b=0.79
scalar ex5_sma_b=-0.232
gen linrad_sma= dgy_sma_b*dose1*exp(ex5_sma_b*(age_exp-5))+1

scalar dgy_lar_b=0.78
scalar ex5_lar_b=-0.2494
...
gen exct3_sin_rad= exp((agecat1_sin_b*agecat1 + agecat2_sin_b*agecat2 + agecat3_sin_b*agecat3 ) * ctpoint) + 1
gen exct3_mul_rad= exp((agecat1_mul_b*agecat1 + agecat2_mul_b*agecat2 + agecat3_mul_b*agecat3 ) * ctpoint) + 1

!Modelled effect modification

scalar dgy_sin_b=0.81
scalar ex5_sin_b=-0.2069
gen linrad_sin= dgy_sin_b*dose1*exp(ex5_sin_b*(age_exp-5))+1


scalar dgyLE_mul_b=0.67 // old, coefficients results different 0.2168 or 0.2173
